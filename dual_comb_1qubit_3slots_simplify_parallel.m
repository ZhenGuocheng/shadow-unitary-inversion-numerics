...
cvx_begin sdp
    ...
    C = PermuteSystems(SW_trans * C * SW_trans', [1 5 6 7 2 3 4 8], [d d d d d d d d]);

    % constraints for comb
    C >= 0;
    PartialTrace(C, [8], [d d d d d d d d])==Tensor(PartialTrace(C,[5 6 7 8], [d d d d d d d d]),(eye(d^3)/d^3));
    ...
    U = RandomUnitary(d);
    U_ketbra = U(:)*U(:)';
    term1 = PartialTrace(PartialTranspose(C, [2 3 4 5 6 7], [d d d d d d d d]) * Tensor(I, PermuteSystems(Tensor(U_ketbra, U_ketbra, U_ketbra), [1 3 5 2 4 6], [d d d d d d]), I), [2 3 4 5 6 7], [d d d d d d d d]);  % C \star |U⟩⟩⟨⟨U|^{⊗3}
